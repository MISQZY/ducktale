...
export function HeaderVines() {
  ...
  useEffect(() => {
    ...
    // One write per frame into four custom properties on a single element; every
    // vine piece reads them back and applies its own depth multipliers, so the
    // cost here does not scale with how many vines are on screen. React never
    // re-renders either — none of this is state.
    const motion = createVineMotion({
      apply: ({ x, y, scroll, fade }) => {
        field.style.setProperty("--vine-x", `${x.toFixed(2)}px`);
        field.style.setProperty("--vine-y", `${y.toFixed(2)}px`);
        field.style.setProperty("--vine-scroll", `${scroll.toFixed(2)}px`);
        field.style.setProperty("--vine-fade", fade.toFixed(3));
      },
